    ...
    postAuthor(util, callbacks)
    {
        ...
        util.conf.$axios.post('/author',
            ...
            .catch(error =>
            {
                if (error.response.status === 403)
                {
                    this.conf.alert.popDanger('invalid authors');
                }
                else
                {
                    this.conf.alert.popDanger('authors upload error');
                }
            })
            .then(res =>
            {
    ...
    postFile(util)
    {
        ...
        util.conf.$axios.post('/file', formData, 
            ...
            .catch(error =>
            {
                if (error.response.status === 403)
                {
                    util.conf.alert.popDanger('you are not allowed to upload this file');
                }
                else
                {
                    util.conf.alert.popDanger('file upload error');
                }
            })
            .then(res =>
            {
                if(res && res.status === 200)
                {
                    util.conf.alert.popSuccess('paper submitted');
                    setTimeout(() =>
                    {
